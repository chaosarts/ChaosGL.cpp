...
namespace ca
{
	namespace gl
	{
		...
		GLint Shader::init ()
		{
			...
			ca::core::info("<Shader> Result of 'init': %i", error);
		...
		GLint Shader::compile()
		{
			...
			error = glGetError();
			ca::core::info("<Shader> Result of 'compile': %i", error);
		...
		GLuint Shader::getId()
		{
			if (_id == 0)
			{
				...
				if (_id == 0) ca::core::warn("<Shader> Invalid type.");
			}
			return _id;
		}
		...
		void Shader::addSource(const char* filename)
		{
			...
			{
				core::warn("<Shader> File %s not found.", filename);
				return;
			}
			...
			{
				fclose(handler);
				core::warn("<Shader> File %s does not support pointer position.", filename);
				return;
			}
